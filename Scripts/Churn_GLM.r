...
for (trial in 1:ntrials){
  ...
  #train
  glmmodel <- glm(Formula, data=train, family=binomial(link="logit"))
  #predict (gives the positive class probabilities directly)
  prob <- predict(glmmodel, newdata=test, type="response")
  ...
  rm(train.index,train,test,glmmodel,prob,truth,threshold,queue_rate,precision,recall)
}
